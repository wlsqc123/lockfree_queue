...
```
- `m_head`: 다음 Pop 위치를 가리킵니다. (Read Index)
- `m_tail`: 다음 Push 위치를 가리킵니다. (Write Index)
- 두 변수 모두 독립적인 캐시 라인에 위치하도록 패딩 처리되어 있습니다.

## 알고리즘 상세

### Push 동작
1. `m_tail`을 로드하여 현재 넣을 위치를 확인합니다.
2. 해당 위치의 `Slot`의 `_generation`을 확인합니다.
   - `_generation == _tail`: 슬롯이 비어있고 내 차례입니다. CAS로 `m_tail`을 증가시켜 슬롯을 예약합니다. 성공 시 데이터를 쓰고 `_generation`을 `_tail + 1`로 업데이트하여 Pop이 가능함을 알립니다.
   - `_generation < _tail`: 큐가 가득 찼거나 다른 스레드가 Pop을 완료하지 않았습니다. 큐가 꽉 찼는지 확인하고 재시도합니다.
   - `_generation > _tail`: 다른 스레드가 이미 이 슬롯을 선점했습니다. 재시도합니다.

```mermaid
flowchart TD
    Start([Push 시작]) --> LoadTail["1. Tail 읽기"]
    LoadTail --> CalcIdx["2. 인덱스 계산<br>(Tail & Mask)"]
    CalcIdx --> ReadGen["3. Generation 읽기"]
    
    ReadGen --> CheckGen{"4. Gen == Tail ?"}
    
    %% CASE 1: 정상 진입
    CheckGen -- "Yes" --> CAS{"5. CAS(Tail++)"}
    CAS -- "성공" --> WriteData["6. 데이터 쓰기"]
    WriteData --> UpdateGen["7. Gen = Tail + 1<br>(공개)"]
    ...
    %% CASE 2: Full or Busy
    CheckGen -- "No (Gen < Tail)" --> CheckFull{"8. 큐 Full?<br>(Tail >= Head + Size)"}
    CheckFull -- "Yes" --> Fail([실패 Return False])
    CheckFull -- "No" --> ReloadTail
    
    %% CASE 3: Others
    CheckGen -- "No (Gen > Tail)" --> ReloadTail
```

### Pop 동작
1. `m_head`를 로드하여 현재 꺼낼 위치를 확인합니다.
2. 해당 위치의 `Slot`의 `_generation`을 확인합니다.
   - `_generation == _head + 1`: 데이터가 준비되었습니다. CAS로 `m_head`를 증가시켜 슬롯을 예약합니다. 성공 시 데이터를 읽고 `_generation`을 `_head + Size`로 업데이트하여 다음 바퀴의 Push가 가능함을 알립니다.
   - `_generation < _head + 1`: 큐가 비었거나 Push가 완료되지 않았습니다. 큐가 비었는지 확인하고 재시도합니다.
   - 그 외: 비정상 상황이거나 경합 상태이므로 재시도합니다.

```mermaid
flowchart TD
    Start([Pop 시작]) --> LoadHead["1. Head 읽기"]
    LoadHead --> CalcIdx["2. 인덱스 계산<br>(Head & Mask)"]
    CalcIdx --> ReadGen["3. Generation 읽기"]
    
    ReadGen --> CheckGen{"4. Gen == Head + 1 ?"}
    
    %% CASE 1: 데이터 있음
    CheckGen -- "Yes" --> CAS{"5. CAS(Head++)"}
    CAS -- "성공" --> ReadData["6. 데이터 읽기"]
    ReadData --> UpdateGen["7. Gen = Head + Size<br>(다음 바퀴 예약)"]
    UpdateGen --> Success([성공 Return True])
    ...
    CheckGen -- "No (Gen < Head + 1)" --> CheckEmpty{"8. 큐 Empty?<br>(Head >= Tail)"}
    CheckEmpty -- "Yes" --> Fail([실패 Return False])
    CheckEmpty -- "No" --> ReloadHead
    
    %% CASE 3: Others
    CheckGen -- "No" --> ReloadHead
```
